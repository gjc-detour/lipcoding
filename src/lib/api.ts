...
export async function fetchInboxItems(
  ...
  const response = await fetch(query ? `/api/inbox?${query}` : "/api/inbox", { credentials: "same-origin" });
...
export async function createInboxItem(item: Partial<InboxItem>): Promise<InboxItem> {
  const response = await fetch("/api/inbox", {
    ...
    headers: jsonHeaders,
    body: JSON.stringify({ type: item.type, raw: item.raw, summary: item.summary, tags: item.tags, due_date: item.due_date }),
  });
  return parseResponse<InboxItem>(response);
}

export async function deleteInboxItem(id: string): Promise<void> {
  const response = await fetch(`/api/inbox/${id}`, { method: "DELETE", credentials: "same-origin" });
  await parseResponse<void>(response);
}

export async function completeInboxItem(id: string): Promise<void> {
  const response = await fetch(`/api/inbox/${id}/complete`, { method: "PATCH", credentials: "same-origin" });
  await parseResponse<void>(response);
}
...
export async function deleteEvent(id: string): Promise<void> {
  const response = await fetch(`/api/events/${id}`, { method: "DELETE", credentials: "same-origin" });
  await parseResponse<void>(response);
}
...
export async function sendChat(message: string, history: ChatMessage[]): Promise<ChatResponse> {
  const response = await fetch("/api/chat", {
    ...
    credentials: "same-origin",
    headers: jsonHeaders,
    body: JSON.stringify({ message, messages: history.map(({ role, content }) => ({ role, content })) }),
  });
